class DoubleCheckValidator < ActiveModel::Validator
    def validate(record)
      @check_event = Event.where('user_id = ? and event_date = ? and end_time > ? and ? > start_time', record.user_id, record.event_date, record.start_time.strftime('%H:%M'), record.end_time.strftime('%H:%M'))
